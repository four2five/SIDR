...
def presentData(data1, data2, plotName, ax, plotLabels1, datasetNum, lns1):
  ...
  myTimesBins = [0, 100, 250, 500, 1000, 1500, 2000, 3000, 4000, 5000, 10000]
               # 0kb, 1kb, 10kb, 100kb, 1MB, 10MB, 50MB, 100MB
  #mySizesBins = [0, 1024, 10240, 51200, 1048576]
  mySizesBins = [0, 1, 10, 25, 50, 75, 100, 120]
  #ax.hist(np.array(data1), 10)
  ax.hist(np.array(data2), mySizesBins, alpha=(0.25 * (datasetNum+1)), histtype='stepfilled')
...
def main():
  ...
  if len(args) != 1:
    ...
    for filename in filesToParse:
      ...
      if formattedInputFile:
        ...
        for line in ins:
          ...
          if plotName is None:
            ...
            #data1.append(long(d1) / 1000000) # convert nanoSeconds to milliseconds
            #data2.append(long(d2) / 1024)    # convert bytes to KB
            data1.append(long(d1)) 
      ...
      # now total up the shuffle size and time

      runningTimeTotal = long(0)
      timeCount = 0
      for dataPoint in data1:
        runningTimeTotal = runningTimeTotal + dataPoint
        timeCount = timeCount + 1

      runningSizeTotal = long(0)
      sizeCount = 0
      for dataPoint in data2:
        runningSizeTotal = runningSizeTotal + dataPoint
        sizeCount = sizeCount + 1


      # convert the counters to seconds and MB
      print plotName, "   ", (runningTimeTotal / 1000000000), " seconds $", (runningSizeTotal / (1024*1024)), " MB"
      print "      ", timeCount, "$", sizeCount
    ...
    #ax.set_title("Per Reduce Task Shuffle Sizes")
