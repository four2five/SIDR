...
public class Utils {
    ...
    //public static final String INPUT_DATATYPE_SIZE = "damasc.input.datatype.size";
    ...
    public static void setVariableShape(Configuration conf, int[] inVariableShape) {
      String inVariableAsString = Utils.arrayToString(inVariableShape);
    ...
    public static void setDatatypeSize(Configuration conf, String datatypeSize) {
      setDatatypeSize(conf, Integer.parseInt(datatypeSize));
    }
    ...
    /*
    public static void setDependencyScheduling(Configuration conf, JobConf job) { 
      job.setDependencyScheduling( conf.getBoolean("mapred.dependency_scheduling", true));
    }
    */
    ...
    public static float getReducerShapeWeight(Configuration conf) {
        //if ( !reducerShapeWeightSet) {
            int numReducers = getNumberReducers(conf);
            //float defaultWeight = (float)1/ (numReducers ); // reasonable default?
            float defaultWeight = (float)1/ (numReducers * 2 ); // reasonable default?
            //reducerShapeWeight = conf.getFloat(REDUCER_SHAPE_WEIGHT, (float)0.20);
            reducerShapeWeight = conf.getFloat(REDUCER_SHAPE_WEIGHT, defaultWeight);
         //   reducerShapeWeightSet = true;
        //}
  ...
  */
  ...
  public static int determineNumberOfReducers(Configuration conf) {  
    ...
    System.out.println("totalOS: " + Utils.arrayToString(totalOutputSpace) + 
  ...
  public static int determineRecordDimensionWeighted(int[] totalOutputShape, 
    ...
    for( int i=0; i<totalOutputShape.length; i++) { 
      ...
      //System.out.println("sizesofar: " + sizeSoFar + " total: " + totalSize + 
  ...
  */
  ...
  {
  /*
    System.out.println("increment start: " + Utils.arrayToString(currentShape) + 
  ...
  */
    ...
    //System.out.println("increment middle: " + Utils.arrayToString(currentShape));
    ...
    //System.out.println("increment end: " + Utils.arrayToString(currentShape));
    return currentShape;
  }
  ...
  public static int[] getInputSplitDependencies(ArrayBasedFileSplit split, 
    ...
    /*
    System.out.println("total input: " + Arrays.toString(totalInputSpace)  + 
                       " ExtShape: " + Arrays.toString(extractionShape) + 
                       " total out: " + Arrays.toString(totalOutputSpace) );
    */
  ...
  public static boolean dependsOn(ArrayBasedFileSplit inputSplit, ArraySpec reducerSpec,
    ...
    return dependsOn(inputSplit, reducerSpec, totalInputSpace, totalOutputSpace, false);
  }
  */
  ...
  public static boolean dependsOn(ArrayBasedFileSplit inputSplit, ArraySpec reducerSpec,
    ...
    while(itr.hasNext()) { 
      ...
      if( Utils.overlaps2(tempSpec, reducerSpec, totalInputSpace, totalOutputSpace, extractionShape, logStuff)) { 
        //System.out.println("\t\tMATCH:" + tempSpec.toString() + ":" + reducerSpec.toString());
        retBool = true;
      }
    }
  ...
  public static boolean overlaps2( ArraySpec inputSplit, ArraySpec reducerSpec,
    ...
    //inputSplit.setVarName("");
    //inputSplit.setFileName("");

    try{ 
      //long extShapeFlattened = Utils.flatten(totalInputSpace, extractionShape); 
      //long aStart = Utils.flatten(totalInputSpace, inputSplit.getCorner()) ;
      //long aEnd = aStart + (Utils.flatten(totalInputSpace, inputSplit.getShape()) -1);
      int[] aStartArray = projectCoordinateIntoIntermediateSpace(inputSplit.getCorner(), 
                                                              extractionShape);
      //long flattenedAStart = Utils.flatten(totalOutputSpace, aStartArray); 
  ...
  public static int getDataTypeSize(Class<?> inputClass) { 
    ...
    //LOG.info("JB, class name: " + inputClass.getSimpleName());

    if (inputClass.getSimpleName().equals("IntWritable")){ 
      //LOG.info("Yo JB, this is an IntWritable");
      retVal = 4;
    } else if (inputClass.getSimpleName().equals("LongWritable")){ 
      //LOG.info("Yo JB, this is an LongWritable");
      retVal = 8;
    }
    /*

    if (inputClass instanceof IntWritable) { 
      retVal = 4;
    } else if (inputClass instanceof LongWritable) { 
      retVal = 8;
    }
    */
    ...
    /*
    switch(inputClass) { 
        case IntWritable: retVal = 4;
          break;
        case LongWritable: retVal = 8;
          break;
        default:
          break;
    */
  ...
  public static FileSystem getFS( Configuration conf ) throws IOException {
    ...
    if( FSType.hdfs == fsType ) { 
      ...
      System.out.println("fs.default.name: " + hdfsNamenodeString);
      ...
      //System.out.println("ceph URI: " + cephURI);
      //fs.initialize(URI.create(cephURI), conf);
      String defaultFSString = conf.get("fs.default.name");
      System.out.println("fs.default.name: " + defaultFSString);
      fs.initialize(URI.create(defaultFSString), conf);
    } else if ( FSType.raw == fsType ) { 
      fs = new RawLocalFileSystem();
      String namenodeString = conf.get("fs.default.name");
      System.out.println("fs.raw.name: " + namenodeString);
      fs.setConf(conf);
      //fs.initialize(URI.create(hdfsNamenodeString), conf);
      //fs = new LocalFileSystem(rfs);
