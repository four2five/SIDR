 ...
 */
public class AverageMapperInt extends Mapper<ArraySpec, MultiVarData, ArraySpec, AverageResultInt> {

private static final Log LOG = LogFactory.getLog(AverageMapperInt.class);
 ...
 */
  public void map(ArraySpec key, MultiVarData inMVD, Context context)
    ...
    try {
      ...
      AverageResultInt aRes = new AverageResultInt();
      ...
      while( dataItr.hasMoreGroups() ) { 
        ...
        aRes.setValue((double)perGroupTotal/perGroupCount, perGroupCount);
      ...
      System.out.println("Caught an exception in AverageMapperInt.map()" + e.toString() );
      e.printStackTrace();
    }
  }
}
