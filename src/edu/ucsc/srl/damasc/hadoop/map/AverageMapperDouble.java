 ...
 */
public class AverageMapperDouble extends Mapper<ArraySpec, MultiVarData, ArraySpec, AverageResultDouble> {

private static final Log LOG = LogFactory.getLog(AverageMapperDouble.class);
    ...
    try {
      ...
      while( dataItr.hasMoreGroups() ) { 
        ...
        while( dataItr.groupHasMoreValues() ) { 
          perGroupTotal += dataItr.getNextValueDouble();
          perGroupCount++;
      ...
      System.out.println("Caught an exception in AverageMapperDouble.map()" + e.toString() );
      e.printStackTrace();
    }
  }
}
