...
public class ArraySpec implements WritableComparable<ArraySpec>, Cloneable {
  ...
  public ArraySpec(int[] corner, int[] shape, 
	...
	if ( null == corner) { 
		...
		throw new IOException("A null shape[] was passed to ArraySpec constructor");
	}
	*/
	// Now that we're using ArraySpec to represent both arrays and single datapoints, 
	// we need to allow for zero-length shapes (a single-data point).
	/*
...
/** 
  ...
  */
   ...
   */
  /*
  public int compareTo(ArraySpec other) {
    int retVal = 0;
  ...
  */
  ...
  public boolean abuts(ArraySpec spec) { 
    ...
    if( 0 == thisShape.length) { 
    	if( (thisCorner[i] + 1) == specCorner[i]) { 
    		//System.out.println("abuts: true.1 " + thisCorner[i] + " + 1 = " + specCorner[i]);
    		return true;
    	} 
    } else { // non-zero length shape
    	if( (thisCorner[i] + thisShape[i]) == specCorner[i] ) { 
    		//System.out.println("abuts:true.2 " + thisCorner[i] + " + " + thisShape[i] + " = " + specCorner[i]);
    		return true;
    	} else {
    		//System.out.println("abuts:false.2 " + thisCorner[i] + " + " + thisShape[i] + " != " + specCorner[i]);
        return false;
      }
    }
  ...
  public void combine( ArraySpec spec ) { 
    ...
    for( i=0; i<getRank(); i++) { 
    	if( this.getCorner()[i] != spec.getCorner()[i]) { 
    		//System.out.println("combine: i: " + i + "  " +  this.getCorner()[i] + 
    		//					" != " + spec.getCorner()[i]);
    		break;
    	} else { 
    		//System.out.println("combine: i: " + i + "  " +  this.getCorner()[i] + 
  ...
  public static void compactList(ArrayList<ArraySpec> arraySpecList) { 
    //int lastSize = arraySpecList.size();
    int lastSize = -1;
    ...
    while( lastSize != arraySpecList.size() ) { 
      ...
      for( int i=0; i<arraySpecList.size() - 1; i++) { 
        ...
        if( current.abuts(next)) { 
          //System.out.println("this " + current.toString() + " does abut " + next.toString());
          current.combine(next);
          arraySpecList.remove(i+1);
        } else { 
          //System.out.println("this " + current.toString() + " does NOT abut " + next.toString());
        }
      }
    }
  }
