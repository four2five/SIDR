 ...
 */
public abstract class NetCDFHDFSCoreFileInputFormat
  ...
  private static final Log LOG = 
      LogFactory.getLog(NetCDFHDFSCoreFileInputFormat.class);
  ...
  protected List<FileStatus> listStatus(JobContext jobC) throws IOException {
    ...
    FSType fsType = Utils.getFSType(jobC.getConfiguration());
    ...
    //String defaultFsName = jobC.getConfiguration().get("fs.default.name", "");
    //LOG.warn("Default fs: " + defaultFsName);
    ...
    for (FileStatus file: files) {
      ...
      String tempPath = file.getPath().toString();
      String newPath = "";

      if (tempPath.startsWith(defaultFsName)) {
        LOG.warn("\tThis starts with the default fs path");
        newPath = tempPath.substring( defaultFsName.length());
        LOG.warn("\tNew path: " + newPath);
      }
      */

      RandomAccessFile raf = null;

      raf = NetCDFHDFSTools.getRAF( jobC.getConfiguration(), file );
      ...
      try {
        ncfile = NetcdfFile.open(raf, file.getPath().toString());
      } catch (IOException e) {
    ...
    if ( Utils.noScanEnabled(conf)) {
      if( Utils.isValid(currentStart, conf)) {  
        Utils.insertNewAs( blockToAS, currentBlock * blockSize, 
                     ...
                     var.getShortName(), fileName, varShape) ); // here
      }
    } else {
      Utils.insertNewAs( blockToAS, currentBlock * blockSize, 
                   new ArraySpec(currentStart, readShape, 
                                 var.getShortName(), fileName, varShape) );
    }
  ...
  */
  ...
  private void genFileSplits(JobContext job, SHFileStatus shFileStatus,
    ...
    FileSystem fs = NetCDFHDFSTools.getFS(job.getConfiguration());
    //initialize the CephFs
    //CephFileSystem cfs = new CephFileSystem();
    //cfs.initialize(URI.create("ceph://null"), job.getConfiguration());
    ...
    RandomAccessFile raf = NetCDFHDFSTools.getRAF( job.getConfiguration(), fileStatus );
    NetcdfFile ncfile = NetcdfFile.open(raf, path.toString());

    long blockSize = fileStatus.getBlockSize();

    System.out.println("File " + fileStatus.getPath() + " has blocksize: " + 
    ...
    switch ( partMode) { 
      case proportional: 
        records = proportionalPartition( dims, variable.getShortName(), 
        ...
        case record:
          records = recordBasedPartition( dims, variable.getShortName(),
     ...
     LOG.info("\t!!!! starting placement for variable " + 
              variable.getShortName() + " with " + records.length + " records !!!!!");
    ...
    System.out.println("\t!!!! done with placement for variable " + 
             variable.getShortName() + " build " + totalArraySpecCount + " array specs !!!!!");
    ...
    while (blockMapIter.hasNext()) {
      ...
      if( null == split) { 
        LOG.warn("Null ArrayBasedFileSplit generated\n" + 
                 "\tpath: " + path + "\n" + 
                 "\tarrays : " );
        for( ArraySpec array: arrays) { 
          LOG.warn("\t" + array.toString());
        }
        for( String host : entry.getKey().getHosts()) { 
          LOG.warn("\thost: " +  host);
        }
    ...
    */
    public Variable getVariable( FileStatus file, Configuration conf) { 
      ...
      try { 
	      ...
	      //NcCephRaf raf = new NcCephRaf( file, conf );

        RandomAccessFile raf = NetCDFHDFSTools.getRAF( conf, file );
	      NetcdfFile ncfile = NetcdfFile.open(raf, path.toString());
	      ...
	      for (Variable variable : variables) {
	        ...
	        if ( !variable.getShortName().equals(varName) && !varName.equals("") )  {
	          //LOG.info(variable.getName() + " != " + varName );
	          continue;
	        } else {
	          LOG.info(variable.getShortName() + " == " + varName);
	          retVar = variable;
	        }
        ...
        if (null == retVar) { 
          LOG.error("Could not find variable: " + Utils.getVariableName(conf) + " in file: " + path.toString());
        }

	    } catch (Exception e) {
	      System.out.println("Caught an exception in NetCDFHDFSCoreFileInputFormat.getVariable()" + 
	                         e.toString() );
	    }
  ...
  //splits = loadSplitsFromCache(files);
