  ...
  private BlockLocation arrayToBlock(Variable variable, ArraySpec array,
    ...
    try {
      ...
      throw new IOException(e.toString() + "\n corner: " + 
                            Arrays.toString(array.getCorner()) + 
                            " shape: " + 
                            Arrays.toString(array.getShape()) + 
                            " sample coord: " + 
                            Arrays.toString(sampleIndex));
    }
    ...
    for (int i = 0; i < sampleSize; i++) {
      ...
      /* 
       * Use the netCDF extension to lookup the offset for this sample
       * coordinate.
       */
      ...
      for (InputSplit split : splits) {
        ...
        for ( ArraySpec spec : tempSplit.getArraySpecList() ) {
          writer.write("File: " + spec.getFileName() + 
                       "\tvar: " + spec.getVarName()); 
          writer.write( "\tcorner: " + Arrays.toString( spec.getCorner())); 
          writer.write( "\t shape: " + Arrays.toString( spec.getShape())); 
          writer.write("\t startOffset: " + Arrays.toString( spec.getLogicalStartOffset()) );
          writer.newLine();
        }
        i++;
      }
