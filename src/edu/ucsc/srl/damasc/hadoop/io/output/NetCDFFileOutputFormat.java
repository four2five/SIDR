...
public class NetCDFFileOutputFormat<K, V> extends FileOutputFormat<K, V> { 
  ...
  protected static class NetCDFWriter<K, V> extends RecordWriter<K,V> { 
	  ...
	  //private ArraySpec _tempArraySpec;
    ...
    private Array _writeArray;
      ...
      this._var = var;
	    ...
	    //this._writeShape = new int[this._totalOutputShape.length];
    ...
    public void setReducerCorner( int[] corner ) { 
      ...
      // we need to re-orient the corner relative to this 
      // reduce task's 
      //corner = Utils.subtractArrayFromAnother(corner, this._reducerWriteCorner);
  ...
  public RecordWriter<K, V> getRecordWriter( TaskAttemptContext job) 
    ...
    // Path pathToRead = new Path(fileToRead);
    ...
    //return new NetCDFWriter<K, V>(ncwFile, var, job);
    return retWriter;
  }
