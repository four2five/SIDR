 ...
 */
public class HolisticResultFloat implements Writable,FloatTypedResult { 
  private float[] _values = null; // the values needed to apply the holisitic function
...
private static final Log LOG = LogFactory.getLog(HolisticResultFloat.class);
  ...
  */
   ...
   */
  public HolisticResultFloat(int neededCount) throws Exception {
    this._values = new float[neededCount];
    this._currentCount = 0;
    this._full = false;
    this._final = false;
  }
   ...
   * definitive result
   */
  public HolisticResultFloat(int neededCount, float value) throws Exception {
    this._values = new float[neededCount];
    this._currentCount = 0;
    this._full = false;
    this._final = false;
  ...
  public void setNeededCount(int neededCount) {
    float[] newArray = new float[neededCount];
  ...
  public void setValue(float value) throws IOException {
    if( this._values.length > 1 && this.isFull()) {
      throw new IOException("ERROR: adding an element to an already " + 
                            "full HolisticResultFloat object." +
                            "Length: " + this._values.length);
    }

    if ( this._final == true ) {
      throw new IOException("ERROR: adding a value to a " +
                            "HolisticResultFloat that has been marked final");
    }
   ...
   */
  public void setFinal( float inValue) throws IOException {
    //System.out.println("start, setFinal: [0] = " + this._values[0] + " inValue: " + inValue);
    this._values = new float[1]; // free up the now useless ._values array
   ...
   */
  public void setHolisticResultFloat( float[] values ) throws IOException {
   ...
   * @param result the HolisticResultFloat object to use to initialize this
   * object
   */
  public void setHolisticResultFloat( HolisticResultFloat result) throws IOException {
    this.setHolisticResultFloat( result.getValues());
    this.setFinal(result.isFinal());
  }

  /**
   * Merges a HolisticResultFloat object into this HolisticResultFloat object
   * @param result the HolisiticResult object to merge into this object
   */
  public void merge( HolisticResultFloat result ) {
    try { 
  ...
  public void write(DataOutput out) throws IOException {
    if( isFinal() ) {
      ...
      out.writeFloat(this._values[0]); // the final result
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    this._values = new float[localArraySize];
