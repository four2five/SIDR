 ...
 */
public class HolisticResultDouble implements Writable,DoubleTypedResult { 
  private double[] _values = null; // the values needed to apply the holisitic function
...
private static final Log LOG = LogFactory.getLog(HolisticResultDouble.class);
  ...
  public Class getWrappedValueClass() { 
    ...
    if (null == arrayClass) { 
      LOG.info("in getWrappedValueClass() and arrayClass is null");
    } else { 
      LOG.info("in getWrappedValueClass(), arrayClass is: " + arrayClass.toString());
    }
    ...
    return Utils.determineWrappedClass(_values.getClass());
  }
  */
   ...
   */
  public HolisticResultDouble(int neededCount) throws Exception {
    this._values = new double[neededCount];
    this._currentCount = 0;
    this._full = false;
    this._final = false;
  }
   ...
   * definitive result
   */
  public HolisticResultDouble(int neededCount, double value) throws Exception {
    this._values = new double[neededCount];
    this._currentCount = 0;
    this._full = false;
    this._final = false;
  ...
  public void setNeededCount(int neededCount) {
    double[] newArray = new double[neededCount];
  ...
  public void setValue(double value) throws IOException {
    if( this._values.length > 1 && this.isFull()) {
      throw new IOException("ERROR: adding an element to an already " + 
                            "full HolisticResultDouble object." +
                            "Length: " + this._values.length);
    }

    if ( this._final == true ) {
      throw new IOException("ERROR: adding a value to a " +
                            "HolisticResultDouble that has been marked final");
    }
   ...
   */
  public void setFinal( double inValue) throws IOException {
    //System.out.println("start, setFinal: [0] = " + this._values[0] + " inValue: " + inValue);
    this._values = new double[1]; // free up the now useless ._values array
   ...
   */
  public void setHolisticResultDouble( double[] values ) throws IOException {
   ...
   * @param result the HolisticResultDouble object to use to initialize this
   * object
   */
  public void setHolisticResultDouble( HolisticResultDouble result) throws IOException {
    this.setHolisticResultDouble( result.getValues());
    this.setFinal(result.isFinal());
  }

  /**
   * Merges a HolisticResultDouble object into this HolisticResultDouble object
   * @param result the HolisiticResult object to merge into this object
   */
  public void merge( HolisticResultDouble result ) {
    try { 
  ...
  public void write(DataOutput out) throws IOException {
    if( isFinal() ) {
      ...
      out.writeDouble(this._values[0]); // the final result
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    this._values = new double[localArraySize];
