 ...
 */
public class MedianReducerDouble extends 
        Reducer<ArraySpec, HolisticResultDouble, ArraySpec, HolisticResultDouble> {

  private static final Log LOG = LogFactory.getLog(MedianReducerDouble.class);
  ...
  public void setup(Context context) throws IOException, InterruptedException { 
    ...
    if( null == conf)  {
      LOG.info("in MedianReducerDouble.setup(), conf is non-existent");
    }
  ...
  public void reduce(ArraySpec key, Iterable<HolisticResultDouble> values, 
    ...
    HolisticResultDouble holResult = null;

    try{ 
      holResult = new HolisticResultDouble(HadoopUtils.calcTotalSize(this._extractionShape));
    } catch ( Exception e ) { 
      e.printStackTrace();
    }
    ...
    if( holResult.isFinal() ) { 
      ...
      holResult.sort();
      double medianValue = holResult.getValue(holResult.getCurrentCount()/2);
