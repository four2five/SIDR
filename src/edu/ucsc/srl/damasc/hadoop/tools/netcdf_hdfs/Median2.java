...
import edu.ucsc.srl.damasc.hadoop.map.IntMapper;
import edu.ucsc.srl.damasc.hadoop.map.ShortMapper;
import edu.ucsc.srl.damasc.hadoop.partition.ArraySpecPartitioner;
//import edu.ucsc.srl.damasc.hadoop.reduce.MedianReducer;
import edu.ucsc.srl.damasc.hadoop.reduce.IntReducer;
import edu.ucsc.srl.damasc.hadoop.reduce.ShortReducer;
...
public class Median2 extends Configured implements Tool {

	public int run(String[] args) throws Exception {

    System.out.println(" in netcdf_hdfs.Median2.run(), args len: " + args.length + " args content ");
    ...
    job.setJarByClass(Median2.class);
    // -jbuck use input data type to set mapper here
    if (DataType.INT == inputDataType) { 
      System.out.println("  Using Int mapper and reducer");
      job.setMapperClass(IntMapper.class);
      job.setReducerClass(IntReducer.class);
    } else if (DataType.SHORT == inputDataType) { 
      System.out.println("  Using Short mapper and reducer");
      job.setMapperClass(ShortMapper.class);
      job.setReducerClass(ShortReducer.class);
    }
    ...
    System.out.println("in netcdf.Median2.main()");
		int res = ToolRunner.run(new Configuration(), new Median2(), args);
		System.exit(res);
	}
}
