...
import edu.ucsc.srl.damasc.hadoop.map.WeightedAverageMapperFloat;
...
public class WeightedAverage extends Configured implements Tool {
    ...
    System.out.println(" in netcdf_hdfs.WeightedAverage.run(), args len: " + args.length + " args content ");
    ...
    for( int i=0; i<args.length; i++) { 
      System.out.println(args[i]);
		...
		if (args.length != 2) {
			System.err.println("Usage: netcdf_hdfs_weighted_average <input> <output>");
			System.exit(2);
		}
    ...
    String jobNameString = "WeightedAverage ";
    ...
    {
      System.out.println("Weights File: " + weightsFileName);
      DistributedCache.addCacheFile(new URI(weightsFileName), conf);
    }
    else
    {
      System.out.println("No weights file specified");
    }
  ...
  public static void main(String[] args) throws Exception {
    System.out.println("in netcdf_hdfs.WeightedAverage.main()");
    int res = ToolRunner.run(new Configuration(), new WeightedAverage(), args);
    System.exit(res);
  }
}
